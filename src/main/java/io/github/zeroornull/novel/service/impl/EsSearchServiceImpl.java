...
public class EsSearchServiceImpl implements SearchService {
    ...
    public RestResp<PageRespDto<BookInfoRespDto>> searchBooks(BookSearchReqDto condition) {
        SearchResponse<EsBookDto> response = esClient.search(s -> {
            ...
            if (!StringUtils.isBlank(condition.getSort())) {
                searchBuilder.sort(o -> o.field(f -> f.field(StringUtils.underlineToCamel(condition.getSort().split(" ")[0])).order(SortOrder.Desc)));
            }
            // 分页
            searchBuilder.from((condition.getPageNum() - 1) * condition.getPageSize()).size(condition.getPageSize());
            // 设置高亮显示
            searchBuilder.highlight(h ->
                    h.fields(EsConsts.BookIndex.FIELD_BOOK_NAME,
        ...
        assert total != null;
        return RestResp.ok(
                PageRespDto.of(condition.getPageNum(),
    ...
    private void buildSearchCondition(BookSearchReqDto condition, SearchRequest.Builder searchBuilder) {
        ...
        BoolQuery boolQuery = BoolQuery.of(b -> {
            ...
            b.must(RangeQuery.of(m ->
                    m.number(n ->
                            n.field(EsConsts.BookIndex.FIELD_WORD_COUNT)
                                    .gt(0d)
                    )
            )._toQuery());
            if (!StringUtils.isBlank(condition.getKeyword())) {
                // 关键字查询
                b.must(q -> q.multiMatch(t -> t.fields(EsConsts.BookIndex.FIELD_BOOK_NAME + "^2",
                        EsConsts.BookIndex.FIELD_AUTHOR_NAME + "^1.8",
                        EsConsts.BookIndex.FIELD_BOOK_DESC + "^0.1").query(condition.getKeyword()))
                );
            ...
            if (Objects.nonNull(condition.getWordCountMin())) {
                b.must(RangeQuery.of(m -> m.number(n ->
                                n.field(EsConsts.BookIndex.FIELD_WORD_COUNT)
                                        .gte(Double.valueOf(condition.getWordCountMin()))
                        )
                )._toQuery());
            }
            if (Objects.nonNull(condition.getWordCountMax())) {
                b.must(RangeQuery.of(m -> m.number(n ->
                                n.field(EsConsts.BookIndex.FIELD_WORD_COUNT)
                                        .lt(Double.valueOf(condition.getWordCountMax()))
                        )
                )._toQuery());
            }
            ...
            if (Objects.nonNull(condition.getUpdateTimeMin())) {
                b.must(RangeQuery.of(m -> m.number(n ->
                                n.field(EsConsts.BookIndex.FIELD_LAST_CHAPTER_UPDATE_TIME)
                                        .gte((double) condition.getUpdateTimeMin().getTime())
                        )
                )._toQuery());
            }
            return b;
